...
OUTPUT_DIR   = 'outputs/AICUP/GRADCAM/RT-DETR'
SCORE_THRESH = 0.3
...
# debug: in cau truc model de chon dung target_layer
print('--- Model structure (top-level) ---')
for i, layer in enumerate(model.model.model):
    print('  [{}] {}'.format(i, type(layer).__name__))

# RT-DETR: backbone nam trong model.model.model
# thuong la: index 0,1,2 la backbone (Conv, AIFI, ...)
#            index -1 la RTDETRDecoder
# target layer: layer truoc decoder, thuong la [-2] hoac [-3]
target_layers = [model.model.model[-2]]
print('Target layer: model.model.model[-2] =', type(model.model.model[-2]).__name__)
