...
# -----------------------------------------------
# CONFIG - chi can thay doi phan nay
# -----------------------------------------------
...
OUTPUT_DIR  = '/home/ai/mmdetection3x/imgs/outputs/aicup/Gradcam/DETR_GT'
...
#OUTPUT_DIR  = '/home/ai/mmdetection3x/imgs/outputs/aicup/Gradcam/DINO_GT'
...
#OUTPUT_DIR  = '/home/ai/mmdetection3x/imgs/outputs/aicup/Gradcam/SWINB_GT'
...
class BackboneNeckWrapper(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.has_neck:
            feat = self.neck(feat)
            return feat[0]
        # Swin khong co neck: tra ve feature cuoi
...
def get_target_layers(model, wrapped):
    ...
    # ResNet: FasterRCNN, CascadeRCNN, DynamicRCNN, FCOS, RetinaNet, Sparse
    if hasattr(backbone, 'layer4'):
        print('Target layer: backbone.layer4[-1]')
        return [wrapped.backbone.layer4[-1]], None

    # SwinTransformer MMDet version: dung 'stages'
    ...
    # SwinTransformer HuggingFace version: dung 'layers'
...
# format file txt: class cx cy w h  (normalized, 1 dong/box)
# -----------------------------------------------
...
# them vao sau dong: full_model = init_detector(...)
