...
# CONFIG
# -----------------------------------------------
MODEL_WEIGHTS = '/home/ai/Cardiomegaly/rfdetr/checkpoint_best_total.pth'
INPUT_DIR     = 'imgs/cardiomegaly'
GT_DIR        = 'imgs/cardiomegaly/ground_truth'
OUTPUT_DIR    = 'imgs/outputs/cardiomegaly/Gradcam/RFDETR'
SCORE_THRESH  = 0.3
...
def dinov2_reshape_transform(tensor, height=None, width=None):
    if tensor.dim() == 3:
        ...
        for h2 in range(1, int(total ** 0.5) + 2):
            if total % h2 == 0:
                w2   = total // h2
                diff = abs(h2 - w2)
                if diff < best_diff:
        ...
        if best is not None:
            h2, w2 = best
            return tensor.reshape(1, h2, w2, C).permute(0, 3, 1, 2).float()
        s   = int(total ** 0.5) + 1
        pad = s * s - total
        flat = tensor.reshape(1, total, C)
        flat = torch.nn.functional.pad(flat, (0, 0, 0, pad))
        return flat.reshape(1, s, s, C).permute(0, 3, 1, 2).float()
    if tensor.dim() == 4:
        return tensor.float()
    return tensor.float()
...
class RFDETRWrapper(torch.nn.Module):
    def __init__(self, rfdetr_model, input_size=560):
        super().__init__()
...
try:
    RFDETR_SIZE = int(rfdetr.size)
except Exception:
    RFDETR_SIZE = 560
print('RF-DETR input size:', RFDETR_SIZE)

wrapped = RFDETRWrapper(rfdetr, input_size=RFDETR_SIZE)
wrapped.eval()
wrapped.to(device)

target_layers = [wrapped.inner.layer[-1].norm2]
print('Target layer: inner.layer[-1].norm2')

means = np.array(rfdetr.means, dtype=np.float32) * 255.0
stds  = np.array(rfdetr.stds,  dtype=np.float32) * 255.0

try:
    class_names = list(rfdetr.class_names.values()) if isinstance(rfdetr.class_names, dict) else list(rfdetr.class_names)
except Exception:
...
with EigenCAM(model=wrapped, target_layers=target_layers,
    ...
    for idx, image_path in enumerate(all_images):
        ...
        try:
            ...
            # preprocess: resize ve RFDETR_SIZE cho EigenCAM
            img_resized  = cv2.resize(img_rgb, (RFDETR_SIZE, RFDETR_SIZE))
            img_norm     = (img_resized.astype(np.float32) - means) / stds
